...
function run(args,{capture=false,allowFailure=false}={}){
  ...
  try{return execFileSync(npx,['wrangler',...args],{encoding:'utf8',stdio:capture?['ignore','pipe','inherit']:'inherit',env:process.env});}
  catch(error){if(allowFailure)return '';throw error;}
}
function jsonFrom(text){const clean=String(text||'').trim();for(let i=0;i<clean.length;i++){if(clean[i]!=='['&&clean[i]!=='{')continue;try{return JSON.parse(clean.slice(i))}catch{}}throw new Error('Wrangler não retornou JSON válido.');}
...
function ensureDatabase(name){let db=listDatabases().find(x=>x.name===name);if(!db){console.log(`Criando D1 ${name}...`);run(['d1','create',name]);db=listDatabases().find(x=>x.name===name)}if(!db)throw new Error(`Não foi possível localizar o D1 ${name} após criação.`);return{name,id:db.uuid||db.id||db.database_id};}
function listProjects(){return asArray(jsonFrom(run(['pages','project','list','--json'],{capture:true})),'result','projects')}
function ensureProject(){const exists=listProjects().some(x=>x.name===PROJECT||x.project_name===PROJECT);if(!exists)run(['pages','project','create',PROJECT,'--production-branch',PRODUCTION_BRANCH]);}

const prod=ensureDatabase(PROD_DB),preview=ensureDatabase(PREVIEW_DB);ensureProject();
...
run(['d1','migrations','apply',PREVIEW_DB,'--remote','--env','preview','--config',configPath]);

const sessionSecret=process.env.SESSION_SECRET||randomBytes(64).toString('hex');
const secrets={ADMIN_PASSWORD:process.env.ADMIN_PASSWORD,SESSION_SECRET:sessionSecret,SESSION_TTL_DAYS:process.env.SESSION_TTL_DAYS||'30'};
...
const secretPath=`${workDir}/pages-secrets.json`;writeFileSync(secretPath,JSON.stringify(secrets));
try{
  console.log('Atualizando secrets de produção...');run(['pages','secret','bulk',secretPath,'--project-name',PROJECT,'--config',configPath]);
  console.log('Atualizando secrets de preview...');run(['pages','secret','bulk',secretPath,'--project-name',PROJECT,'--env','preview','--config',configPath]);
...
console.log('Publicando Pages...');run(deployArgs);console.log(`Deploy concluído: https://${PROJECT}.pages.dev`);
