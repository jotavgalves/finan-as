...
type Page = 'home'|'flow'|'bills'|'planning'|'more'|'admin';

function currentMonth() { return new Date().toISOString().slice(0, 7); }
function initialPage(): Page {
  const p = new URLSearchParams(location.search).get('page') as Page | null;
  return p && ['home','flow','bills','planning','more','admin'].includes(p) ? p : 'home';
}
...
export function App() {
  ...
  const titles = useMemo(() => ({ home:['Posição financeira','Visão geral'], flow:['Navegação temporal','Fluxo'], bills:['Operação diária','Contas'], planning:['Solvência','Planejamento'], more:['Estrutura','Mais'], admin:['Segurança e integridade','Admin'] } as const), []);
  ...
  const pageNode = page === 'home' ? <Dashboard key={revision} month={month} onNew={setEntryKind} onOpenPlanning={()=>setPage('planning')} />
    : page === 'flow' ? <Flow key={revision} onSettle={setSettling} onEdit={e=>{setEditing(e);setEntryKind(e.kind)}} onDelete={remove} />
    : page === 'bills' ? <Bills key={revision} onSettle={setSettling} onEdit={e=>{setEditing(e);setEntryKind(e.kind)}} onDelete={remove} />
    : page === 'planning' ? <Planning key={revision} month={month} />
    : page === 'admin' ? <Admin key={revision} onBack={()=>setPage('more')} />
    : <More key={revision} onAdmin={()=>setPage('admin')} />;

  const nav: Array<[Page,string]> = [['home','Início'],['flow','Fluxo'],['bills','Contas'],['planning','Planejar'],['more','Mais'],['admin','Admin']];
