  ...
  return <div class="modalBackdrop" onClick={e => e.currentTarget === e.target && props.onClose()}>
    <form class="modal" onSubmit={submit}>
      ...
      <div class="formGrid">
        ...
        <div class="field"><label>Data / vencimento</label><input class="input" required type="date" value={dueDate} onInput={e => setDueDate((e.target as HTMLInputElement).value)} /></div>
        <div class="field"><label>{props.kind === 'income' ? 'Fonte' : 'Categoria'}</label><input class="input" value={label} onInput={e => setLabel((e.target as HTMLInputElement).value)} placeholder={props.kind === 'income' ? 'Salário principal' : 'Moradia'} /></div>
        <div class="field"><label>Conta financeira</label><select class="select" value={accountId} onChange={e => setAccountId((e.target as HTMLSelectElement).value)}><option value="">Sem conta definida</option>{accounts.map(a => <option value={a.id}>{a.name}</option>)}</select></div>
        ...
        {!props.entry && <div class="field"><label>Recorrência</label><select class="select" value={recurring ? 'yes' : 'no'} onChange={e => setRecurring((e.target as HTMLSelectElement).value === 'yes')}><option value="no">Não se repete</option><option value="yes">Se repete</option></select></div>}
