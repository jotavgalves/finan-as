...
export function EntryList(props: {
  ...
  return <div class="list">
    {props.entries.map(entry => {
      ...
      const cardLinked=!!entry.cardPurchaseId;
      return <div class="row" key={entry.id}>
        <div class="rowDate">{dateLabel(entry.dueDate)}</div>
        <div class="rowMain">
          <strong>{entry.description}</strong>
          <small>{entry.categoryName || entry.incomeSourceName || 'Sem categoria'} · {entry.accountName || 'Sem conta'} · {status}{cardLinked?' · parcela de cartão':''}</small>
          {entry.settledCents > 0 && entry.status !== 'done' && <small>{money(entry.settledCents)} liquidado · {money(outstanding)} restante</small>}
          {cardLinked&&<small>Valor/data/parcelamento são editados em Mais → Admin → Compras.</small>}
        </div>
        ...
        <div class="rowAction" style="display:flex;gap:6px;flex-wrap:wrap;justify-content:flex-end">
          ...
          {!cardLinked&&props.onEdit && <button class="btn" onClick={() => props.onEdit?.(entry)}>Editar</button>}
          {!cardLinked&&props.onDelete && <button class="btn danger" onClick={() => props.onDelete?.(entry)}>Excluir</button>}
        </div>
      </div>;
    })}
  </div>;
